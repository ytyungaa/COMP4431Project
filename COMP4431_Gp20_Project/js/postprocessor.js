                        ...
                        plugins: {
                            dragData: {
                                ...
                                onDrag: function (e, datasetIndex, index, value) {
                                    ...
                                    if (value.data == "end of attack" && value.x > data[index+1].x){
                                        value.x = data[index+1].x;
                                        return false;
                                    }


                                    if (value.data == "end of hold" && value.x < data[index-1].x){
                                        value.x = data[index-1].x;
                                        return false;
                                    }

                                    if (value.data == "end of hold" && value.x > data[index+10].x){
                                        value.x = data[index+10].x;
                                        return false;
                                    }

                                    if (value.data == "start of sustain" && value.x < data[index-10].x){
                                        value.x = data[index-10].x;
                                        return false;
                                    }

                                    if (value.data == "start of sustain" && value.x > data[index+1].x){
                                        value.x = data[index+1].x;
                                        return false;
                                    }

                                    if (value.data == "end of sustain" && value.x < data[index-1].x){
                                        value.x = data[index-1].x;
                                        return false;
                                    }
                                ...
                                onDragEnd: function (e, datasetIndex, index, value) {
                                    ...
                                    if (value.data == "end of hold") {
                                        var newVal = parseFloat((value.x - attackPeriod).toFixed(2));
                                        if (newVal < 0) newVal=0;
                                        $("#ahdsr-hold-duration").attr(("data-p" + pass), newVal);

                                        $("#ahdsr-hold-duration").val(newVal);
                                        $("#ahdsr-hold-duration").change();
                                    }

                                    if (value.data == "start of sustain") {
                                        var newVal = parseFloat((value.x - startOfDecay).toFixed(2));
                                        if (newVal < 0) newVal=0;
                                        $("#ahdsr-decay-duration").attr(("data-p" + pass), newVal);

                                        $("#ahdsr-decay-duration").val(newVal);

                                        var val2 = parseFloat((value.y).toFixed(3)) * 100;
                                        if (val2 < 0) val2=0;
                                        $("#ahdsr-sustain-level").attr(("data-p" + pass), parseFloat(val2.toFixed(1)));
                                        $("#ahdsr-sustain-level").val(parseFloat(val2.toFixed(1)))
                                    ...
                                    if (value.data == "end of sustain") {
                                        var val = parseFloat((duration - value.x).toFixed(2));
                                        if (val < 0) val=0;
                                        $("#ahdsr-release-duration").attr(("data-p" + pass), val);

                                        $("#ahdsr-release-duration").val(val);
